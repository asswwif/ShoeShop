    ...
    public class SalesService
    {
        public DataTable GetSalesHistory(DateTime startDate, DateTime endDate)
        {
            ...
            try
            {
                ...
                // Запит на отримання даних про чек, клієнта та працівника
                string query = @"
                    SELECT
                        S.sale_id,
                        S.sale_datetime,
            ...
            {
                MessageBox.Show($"Помилка завантаження історії продажів: {ex.Message}", "Помилка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                DbConection.CloseDB();
            }
